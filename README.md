...
Then install the template directly from the repository:

```text
npm i --save-dev EskelCz/jsdoc-minimal
```

Now you can run:

```text
npx jsdoc path/to/source/files -t node_modules/jsdoc-minimal -r README.md
```
...
```json
  "opts": {
    "template": "node_modules/jsdoc-minimal"
  }
```
...
```text
git clone https://github.com/EskelCz/jsdoc-minimal.git
cd jsdoc-minimal
